...
public class DimpLinkedList {
	...
	/*
	* Create a drawing area that is width x height so points can 
	* have values between 0 and width/height.
	...
	*/
	...
	/**
	* Creates a "fake" meningless list from a constant array.
	* used for debug
	...
	*/
	...
	public String toString() {
		...
		StringBuilder str4 = new StringBuilder("o index:    "); // original index
		...
		while (ptr != null) {
			...
			shape.drawLine((int)p1.getX(), (int)p1.getY(), 
							(int)p2.getX(), (int)p2.getY(), 
							 c, 2.0, l);
//			System.out.println( (int)p1.getX()+" "+ (int)p1.getY()+" "+ (int)p2.getX()+" "+ (int)p2.getY()); // debug
			p1 = p2;
			ptr = ptr.next;
		}
		shape.repaint();
